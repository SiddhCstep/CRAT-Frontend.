...
export class HomeComponent {
  ...
  NumberBOXforShowRiskValnarbility: (number | string)[] = [];
  ...
  //   console.log(this.timestampforexposure);
  //   console.log(this.typeOfHazard);


  //   const isHistorical = this.timestampforexposure ;
  //   const isFuture = this.timestampforexposure ;
  //   const isProbability = this.typeOfHazard ;
  //   const isSpatial = this.typeOfHazard;
  ...
  //         switch (option.value) {
  //           case 'Drought':
  //             tooltip = 'Droughts: This data assesses the probability of drought occurrence using 30 years of monthly precipitation data (1991-2022). Only severe and extreme drought cases as defined by the Standardized Precipitation Index (SPI) were considered.';
  //             break;
  //           case 'Heatwave':
  //             tooltip = 'Heatwaves: The probability of heatwave events is calculated based on 30 years of daily temperature data (1991-2022). This measure considers the number of years in which at least one heatwave event has occurred. A heatwave event is defined as two consecutive days of ≥ 37°C for the coastal districts of Puducherry, Karaikal and Yanam and ≥ 32°C for the hilly district of Mahe.';
  //             break;
  //           case 'Sea level rise':
  //             tooltip = 'Sea Level Rise: The probability of exceedance sea level rise is assessed using historical data from the closest tide stations for each district over specified periods. For Pondicherry and Karaikal districts, data from the Chennai tide gauge station (1968-2012) was used to compute the probability of occurrence exceedences of sea level rise in the region. For Yanam and Mahe, data from the Visakhapatnam tide gauge station (1972-2021) and the Kochi tide gauge station (1977-2021) was used, respectively.';
  //             break;
  //           case 'Flood':
  //             tooltip = 'Flood: To calculate the probability of occurrence of floods, return periods have been calculated using 30 years of historical rainfall data (1991-2022). Return periods provide a statistical measure of how often a flood of a certain magnitude or severity can be expected to occur.';
  //             break;
  //         }
  //       } else if (isFuture) {
  //         switch (option.value) {
  //           case 'Drought':
  //             tooltip = 'Droughts: The Representative Concentration Pathway (RCP) 4.5 scenario is used to project future droughts. The Intergovernmental Panel on Climate Change (IPCC) describes this scenario in which greenhouse gas emissions peak around 2040 and then decline, stabilizing radiative forcing at 4.5 watts/m2 by the year 2100 without exceeding that value. This moderate scenario is simulated using the Global Change Assessment Model (GCAM).';
  //             break;
  //           case 'Heatwave':
  //             tooltip = 'Heatwaves: The Representative Concentration Pathway (RCP) 4.5 scenario is used to project future heatwaves. The Intergovernmental Panel on Climate Change (IPCC) describes this scenario in which greenhouse gas emissions peak around 2040 and then decline, stabilizing radiative forcing at 4.5 watts/m2 by the year 2100 without exceeding that value. This moderate scenario is simulated using the Global Change Assessment Model (GCAM).';
  //             break;
  //           case 'Sea level rise':
  //             tooltip = 'Sea-level rise: The Shared Socioeconomic Pathways (SSPs) 2-4.5 is used to forecast future sea-level rise. SSP 2-4.5 is characterized as a "Middle of the Road," which represents a scenario with moderate challenges for both mitigating and adapting to climate change. In this scenario, global development follows a trajectory where social, economic, and technological trends largely continue along historical patterns and emissions are at the current levels till 2050 followed by a reduction, but never reaching net-zero by 2100.';
  //             break;
  //           case 'Flood':
  //             tooltip = 'Flood: The Representative Concentration Pathway (RCP) 4.5 scenario is used to forecast future floods. The Intergovernmental Panel on Climate Change (IPCC) describes this scenario in which greenhouse gas emissions peak around 2040 and then decline, stabilizing radiative forcing at 4.5 watts/m2 by the year 2100 without exceeding that value. This moderate scenario is simulated using the Global Change Assessment Model (GCAM).';
  //             break;
  //         }
  //       }
  //     } else if (isSpatial) {
  ...
  //         switch (option.value) {
  //           case 'Drought':
  //             tooltip = 'Drought: This data presents the spatial extent of drought. Given the limited geographical area of each region within Puducherry U.T, due to limitations of IMD rainfall gridded data resolution (re-gridded to 0.25° X 0.25°), the spatial extent of droughts is uniform over each region.';
  //             break;
  //           case 'Heatwave':
  //             tooltip = 'Heatwave: This data presents the spatial extent of heatwave events. Given the limited geographical area of each region within Puducherry U.T, the spatial extent of heatwaves is uniform over each region. The extent appears uniform across each region within Puducherry U.T, due to limitations of IMD temperature gridded data resolution (re-gridded to 0.25° X 0.25°).';
  //             break;
  //           case 'Sea level rise':
  //             tooltip = 'Sea-level rise: Sea level inundation mapping for the four regions of Puducherry UT employed a comprehensive methodology integrating an elevation-based bathtub model.';
  //             break;
  //           case 'Flood':
  //             tooltip = 'Flood: The spatial extent of flood for return periods 2, 5, 10, 50, and 100 years was derived through a multi-tier process using the HEC–RAS 2D hydraulic model.';
  //             break;
  //         }
  //       } else if (isFuture) {
  //         switch (option.value) {
  //           case 'Drought':
  //             tooltip = 'Drought: This data presents the spatial extent of drought based on the IPCC RCP 4.5 scenario. Given the limited geographical area of each region within Puducherry U.T, the spatial extent of droughts is uniform over each region. To maintain uniformity, CMIP-5 model outputs with varied resolutions were re-gridded to 0.25° × 0.25°.';
  //             break;
  //           case 'Heatwave':
  //             tooltip = 'Heatwave: This data presents the spatial extent of heatwave based on the IPCC RCP 4.5 scenario. Due to the coarse resolution of CMIP5 gridded data (downscaled and re-gridded to 0.25° X 0.25°), the spatial extent of heatwaves appears uniform across each region within Puducherry U.T. To maintain uniformity, CMIP-5 model outputs with varied resolutions were re-gridded to 0.25° × 0.25°.';
  //             break;
  //           case 'Sea level rise':
  //             tooltip = 'Sea-level rise: SLR induced inundation mapping for the four regions of Puducherry UT employed a comprehensive methodology integrating an elevation-based bathtub model using the IPCC SSP2-4.5 scenario.';
  //             break;
  //           // Flood retains original tooltip if no Future Spatial definition
  //         }
  //       }
  //     }
    ...
    return this.hazardoptions.map(option => {
      ...
      if (isProbability) {
        ...
        if (isHistorical) {
          switch (option.value) {
            case 'Drought':
              newTooltip = 'This data presents the spatial extent of drought. The extent appears uniform across Given the limited geographical area of each region within Puducherry U.T, due to limitations of IMD rainfall gridded data resolution (re-girdded to 0.25° X 0.25°)the. spatial extent of droughts is uniform over each region The SPI value for Moderate drought is in the range of -1 to -1.49. ';
              break;
            case 'Heatwave':
  ...
  onMapReady(map: any) {
    ...
    const layersControl = L.control.layers(this.layersControlone.baseLayers, this.layersControlone.overlays, {
      position: 'bottomleft'
    }).addTo(map);
    ...
    polylineMeasure.addTo(map); // Add to the map instance
  ...
  onMapReadyTwo(map: any) {

    this.map = map;
    map.options.minZoom = 11;
    map.options.maxZoom = 15;

    const layersControl = L.control.layers(this.layersControltwo.baseLayers, this.layersControltwo.overlays, {
      position: 'bottomleft'
    }).addTo(map);
  ...
  onMapReadyThree(map: any) {

    this.map = map;
    map.options.minZoom = 12;
    map.options.maxZoom = 15;

    const layersControl = L.control.layers(this.layersControlthree.baseLayers, this.layersControlthree.overlays, {
      position: 'bottomleft' // Move the control to the bottom-left
    }).addTo(map);
  ...
  onMapReadyFour(map: any) {
    this.map = map;
    map.options.minZoom = 12;
    map.options.maxZoom = 15;

    const layersControl = L.control.layers(this.layersControlfour.baseLayers, this.layersControlfour.overlays, {
      position: 'bottomleft' // Move the control to the bottom-left
    }).addTo(map);
  ...
  loadGeoJsonData(): Promise<void> {
    return this.authservice.loadGeoJsonData()
      .then((data: any) => {
        ...
        console.log(this.selectedOpacity);
  ...
  exposurepercentagevaluenewpercentage = {
  ...
  //   console.log(this.typeOfHazard, this.hazard,

  //     this.floodSlidervaluetoshownonselectedindicator,
  ...
  // printMap() {
  //   setTimeout(() => {
  //     window.dispatchEvent(new Event('resize')); // Force Leaflet to resize
  //     window.print();
  //   }, 500); // Delay ensures dropdown expands before printing
  // }
  ...
  expandBox(boxName: string) {
    this.authservice.setBoxName(boxName);
  ...
  shrinkBox(event: Event) {
    this.expandedYes = true
    this.testvar = true;
    event.stopPropagation();
    this.expandedBox = '';
    this.mapscale = false;
    // //console.log("srink box function called.");
  ...
  //   if (value.length > 0) {
  //     this.authservice.getVulnerabilityinduxvalue(this.sectoroption, selectedIndicatorsObject, headers, "calling from home")
  //       .subscribe((data: { district_name: string; vulnerability_index: number; }[]) => {
  //         // Handle the response data
  //         this.savedatatopassinerror = data;

  //         const vulnerabilityIndices: number[] = [];

  //         data.forEach((district: { district_name: string; vulnerability_index: number; }) => {
  //           // Round the index to three decimal places
  //           const roundedIndex = parseFloat(district.vulnerability_index?.toFixed(3));
  //           const color = getColor(district.vulnerability_index);

  //           // Update the map with the district name and color
  //           this.updateMap(district.district_name, color, 1);

  //           // Push rounded vulnerability index to numberBoxes array
  //           this.numberBoxes.push(roundedIndex);

  //           // Push the color to colorBoxes array
  //           this.colorBoxes.push(color);

  //           // Store the vulnerability index in the array
  //           vulnerabilityIndices.push(district.vulnerability_index);

  //           //console.log(district.vulnerability_index, color);
  //         });

  //         // You can now use vulnerabilityIndices for further processing
  //         //console.log('All vulnerability indices:', vulnerabilityIndices);

  //         this.NumberBOXforShowRiskValnarbility = vulnerabilityIndices;
  ...
  onSelectionChangeVulnerabilityoptions(value: any) {
    ...
    if (value.length > 0) {
      this.authservice.getVulnerabilityinduxvalue(this.sectoroption, selectedIndicatorsObject, headers, "calling from home")
        .subscribe((data: { district_name: string; vulnerability_index: number | null; }[]) => {
          this.savedatatopassinerror = data;
          const vulnerabilityIndices: (number | string)[] = [];

          data.forEach((district: { district_name: string; vulnerability_index: number | null; }) => {
            let roundedIndex: string | number | null = district.vulnerability_index;
            let color = getColor(district.vulnerability_index);

            if (district.vulnerability_index === 5) {
              roundedIndex = "Saline";  // Show "Saline"
              color = "#808080"; // Grey color
              district.vulnerability_index = null; // Set to null after displaying
            } else if (district.vulnerability_index === null || district.vulnerability_index === undefined) {
              roundedIndex = "NA";  // Show "NA"
              color = "#FFFFFF"; // White color
            } else {
              roundedIndex = parseFloat(district.vulnerability_index.toFixed(3)); // Round normally
            }

            this.updateMap(district.district_name, color, 1); // Update map

            this.numberBoxes.push(roundedIndex); // Store number, "Saline", or "NA"
            this.colorBoxes.push(color); // Store color
            vulnerabilityIndices.push(roundedIndex);
          });
    ...
    function getColor(vulnerability_index: number | null) {
      if (vulnerability_index === 5) return "#808080"; // Grey for Saline
      if (vulnerability_index === null || vulnerability_index === undefined) return "#FFFFFF"; // White for NA
  ...
  sliderControlForLayerOpacity = new FormControl(this.initialOpacityIndex); // Default at 50%
  ...
  onSelectionChangeExposureoptions(urlhalfpart: any) {
    ...
    const polygonIndicators = [
      'Net Sown Area', 'Mangroves', 'Cattle',
      'Livestock population', 'Green Space', 'Municipal urban area', 'Waterbody', 'Canal', 'Drainage Network', 'River'
    ...
    const lineIndicators = ['Railway line', 'Road network', 'Check dam', 'Drainage Network'];
  ...
  polylineStyle(feature: any) {
    ...
    return {
      ...
      fillColor: color,                  // Fill color             // Keep it fixed or dynamic if needed
      weight: 1,
      fillOpacity: 1,
  ...
  updateBoxes(option: string, timestamp: string, floodslidervalue: string) {
    ...
    if (option === "Drought") {
      ...
      if (timestamp === "Historical") {
        ...
        this.numberBoxes = [45, 44, 39, 0];
      } else if (timestamp === "Future") {
        this.colorBoxes = ["#724f00", "#9d741a", "#c9a243", "#ffce81"];
        this.numberBoxes = [87, 54, 42, 6];
      }
    } else if (option === "Sea level rise") {
      this.HeatwaveBOX = true;
      this.droughtBox = false;
  ...
  showriskbutton() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      let numericVulnerability = typeof valnarbility === "number" ? valnarbility : 0;

      const cubeRoot = Math.pow(hazardFlood * exposure * numericVulnerability, 1 / 3);
  ...
  //   const sliderValue = (event.target as HTMLInputElement).valueAsNumber;
  //   const selectedStop = this.opacityStops[sliderValue];
  //   console.log('Selected Stop:', selectedStop); // Logs the corresponding opacity stop
  // }
